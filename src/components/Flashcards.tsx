...
export default function Flashcards({
  ...
  const [stack, setStack] = useState<Word[]>(() =>
    deck.filter((w) => !learned.has(w.id)),
  );
  ...
  const learnedCount = deck.reduce(
    (n, w) => (learned.has(w.id) ? n + 1 : n),
    0,
  );
  ...
  const card = stack.length
    ? stack[Math.min(index, stack.length - 1)]
    : undefined;
    ...
    >
      ...
      {!card ? (
        ...
        >
          ...
          >
            {Math.min(index, stack.length - 1) + 1} of {stack.length} · tap card
            to reveal
          </div>
        </div>
      )}
...
function AllCaughtUp({
  ...
  return (
    ...
    >
      ...
      >
        You've learned all {total} words at this level. Lower your level in
        Settings for more, or review what you know.
      </div>
